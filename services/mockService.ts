...
const MOCK_RESPONSES_BY_LANGUAGE: Record<string, string[]> = {
  ...
  'ja-JP': [
    ...
    `では説明します。画面には4つの要素があります：私、「ミオン」あなたの頼もしい仲間です　HAHA～.
・重要な情報やグラフを表示する「情報ボックス」
・音声ON/OFF、字幕、言語変更や生成画像
・動画のダウンロードメニュー、音声入力用の「操作ボタン」
・そして会話用の「チャット」です。おっと！

　お待たせしました。スキャンが終了しました！
皮膚炎の症状も以前より緩和傾向にあります。
また、現在のあなたの状態に合わせて抗酸化作用を高めたり、皮膚の免疫バランスを整える硫化水素を含んだ温泉を準備します。
スキャンしたあなたのプロフィールはこちらです：
